    ...
    <div className="page">
      <div className="page-head">
        ...
        <div className="page-sub">
          Live claims intelligence · {new Date().toLocaleDateString('en-GB', { weekday:'long', day:'numeric', month:'long' })}
        </div>
      </div>

      {loading ? (
        <div className="kpi-grid">
          {[...Array(5)].map((_, i) => <div key={i} className="skel" style={{ height: 88 }} />)}
        </div>
      ) : (
        <div className="kpi-grid">
      ...
      <div className="grid-2">
        <div className="card">
          <div className="card-head">
            ...
            <span className="card-title">Claims by type</span>
          </div>
            ...
            <ResponsiveContainer width="100%" height={160}>
              <PieChart>
                <Pie data={statusData} cx="50%" cy="50%"
                  innerRadius={48} outerRadius={72} paddingAngle={3} dataKey="v">
      ...
      <div className="card" style={{ marginBottom:14 }}>
        <div className="card-head">
          <TrendingUp size={15} className="card-icon" />
          <span className="card-title">Claims volume trend</span>
        </div>
        <div className="card-body">
          <ResponsiveContainer width="100%" height={160}>
      ...
      <div className="card">
        <div className="card-head">
          <MapPin size={15} className="card-icon" />
          <span className="card-title">Average fraud score by region (%)</span>
        </div>
        <div className="card-body">
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={regionData} layout="vertical" barSize={12}>
              <CartesianGrid strokeDasharray="3 3" stroke="#f0f2f5" horizontal={false} />
              <XAxis type="number" domain={[15,25]} tick={{ fill:'#9ca3af', fontSize:10 }}
                axisLine={false} tickLine={false} tickFormatter={v => `${v}%`} />
              <YAxis type="category" dataKey="r" tick={{ fill:'#4b5563', fontSize:11 }}
                axisLine={false} tickLine={false} width={85} />
              <Tooltip {...TT} formatter={v => [`${v}%`, 'Fraud score']} />
              <Bar dataKey="s" radius={[0,5,5,0]}>
                {regionData.map((_, i) => (
                  <Cell key={i} fill={`hsl(${220 - i * 8}, ${85 - i * 3}%, ${55 - i * 2}%)`} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
...
const EXAMPLES = [
  'Which regions have highest fraud scores?',
  'What does AXA policy say about claim time limits?',
  'Average resolution time — litigated vs normal?',
  'Score claim CLM0000042',
  'Adjusters with most high-risk claims',
...
function Chat() {
  ...
  const send = useCallback(async (q) => {
    ...
    try {
      ...
      setMsgs(m => [...m, { role: 'ai', content: r.answer, route: r.route, sources: r.sources || [] }])
    } catch(e) {
      setMsgs(m => [...m, { role: 'ai', content: `Error: ${e.message}`, route: 'sql', sources: [] }])
    }
    setLoading(false)
  }, [loading])
  ...
  return (
    <div className="chat-wrap">
      <div className="chat-header">
        ...
        <div className="page-sub">Natural language queries across claims data, policy documents, and ML models</div>
      </div>
      ...
      {msgs.length === 0 && !loading ? (
        <div className="chat-empty">
          ...
          <div className="chat-empty-sub">
            Ask anything about your claims portfolio, policy terms, or get an instant risk score for any claim.
          </div>
          <div className="chips">
            {EXAMPLES.map((ex, i) => (
              ...
              <div className={`bubble ${msg.role}`}>
                {msg.role === 'ai' && msg.route && (
                  <div className="bubble-top">
                    ...
                    <span style={{ fontSize:'0.72rem', color:'#9ca3af' }}>InsuredAI</span>
                  </div>
                )}
      ...
      <div className="chat-bottom">
        ...
        <div className="input-box">
          <textarea className="input-ta"
            placeholder="Ask anything about claims, policies, or specific claim IDs..."
            value={input}
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      <div className="analyser-top">
        ...
        <div className="hint">Try: CLM0000001 · CLM0000042 · CLM0000100 · CLM0001234</div>
      </div>

      <div className="analyser-body">
        {error && (
          <div className="err"><AlertCircle size={16} />{error}</div>
        )}
          ...
          <>
            ...
            <div className="score-row">
              <div className="sc">
                ...
                <div className={`sc-flag ${result.fraud_flag ? 'flag-bad' : 'flag-ok'}`}>
                  {result.fraud_flag
                    ? <><AlertTriangle size={12} /> Flagged for review</>
                    : <><CheckCircle size={12} /> No fraud indicators</>}
                </div>
              </div>
              ...
              <div className="sc">
                ...
                <div className={`sc-flag ${result.litigation_flag ? 'flag-warn' : 'flag-ok'}`}>
                  {result.litigation_flag
                    ? <><AlertCircle size={12} /> Litigation risk elevated</>
                    : <><CheckCircle size={12} /> Low litigation risk</>}
                </div>
              </div>
            ...
            <div className="grid-2">
              <div className="card">
                <div className="card-head">
                  <Activity size={15} className="card-icon" />
                  <span className="card-title">Risk profile radar</span>
                </div>
                <div className="card-body">
...
function Analytics() {
  const byType = [
    ...
    { t:'Vehicle',   f:12, l:8  }, { t:'Health',   f:6,  l:10 },
    { t:'Property',  f:8,  l:6  },
  ]
  const resolution = [
    ...
    <div className="page">
      ...
      <div className="grid-2">
        <div className="card">
          <div className="card-head">
            <BarChart3 size={15} className="card-icon" />
            <span className="card-title">Fraud vs litigation rate by type</span>
          </div>
          <div className="card-body">
            <ResponsiveContainer width="100%" height={210}>
        ...
        <div className="card">
          <div className="card-head">
            <Clock size={15} className="card-icon" />
            <span className="card-title">Median resolution days by status</span>
          </div>
          <div className="card-body">
            <ResponsiveContainer width="100%" height={210}>
      ...
      <div className="card">
        <div className="card-head">
          <Sparkles size={15} className="card-icon" />
            ...
            {perf.map(({ l, v, c, i: Icon }) => (
              <div key={l} className="perf-row">
                <div className="perf-left">
                  <Icon size={15} style={{ color: c, flexShrink: 0 }} />{l}
                </div>
